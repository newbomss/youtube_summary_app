...
API_KEY = os.getenv("YOUTUBE_API_KEY")  # .env 파일에서 API 키 가져오기
...
def fetch_video_data(channel_id, keyword):
    try:
        ...
        youtube = build("youtube", "v3", developerKey=API_KEY)  # API 키 반영
...
# Streamlit 상태 관리: API 호출 여부 확인
if 'video_data_list' not in st.session_state:
    st.session_state.video_data_list = None

# 버튼 클릭 시에만 데이터 로드
if st.button("Load Video Data") or st.session_state.video_data_list is None:
    video_data_list = fetch_video_data(channel_id, keyword)
    st.session_state.video_data_list = video_data_list
